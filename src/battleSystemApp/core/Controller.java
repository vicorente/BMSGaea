...
public class Controller {
	...
	public void start(String appConfigurationLocation, Dimension appSize)
			throws Exception {
		this.appTitle = Configuration
				.getStringValue(Constants.APPLICATION_DISPLAY_NAME);
	...
	public Frame getFrame() {
		...
		return Util
				.findParentFrame((Container) getRegisteredObject(Constants.APP_PANEL));
	}
	...
	public NetworkActivitySignal getNetworkActivitySignal() {
		return (NetworkActivitySignal) this
	...
	public void redraw() {
		if (this.getWWd() != null) {
			this.getWWd().redraw();
			// si no se ha definido el tamanio, lo hacemos pantalla completa
	...
	public Layer addInternalActiveLayer(Layer layer) {
		// Internal Active layers are not shown in the layer tree but are shown
		// in the active layers list
	...
	public void moveToLocation(LatLon location) {
		...
		double newAlt = curAlt != null && curAlt <= GOTO_ALTITUDE ? curAlt
				: GOTO_ALTITUDE;
		this.moveToLocation(new gov.nasa.worldwind.geom.Position(location,
				newAlt));
	}
	...
	public void moveToLocation(Position position) {
		...
		if (globe != null && view != null) {
			((OrbitViewInputHandler) view.getViewInputHandler())
					.addPanToAnimator(position, Angle.ZERO, Angle.ZERO,
	...
	public String setStatusMessage(String message) {
		return this.getStatusPanel() != null ? this.getStatusPanel()
	...
	public void showErrorDialog(Exception e, String title, String message,
			Object... args) {
		this.showMessageDialog(formatMessage(e, message, args), title,
				JOptionPane.ERROR_MESSAGE);
	}

	public void showErrorDialogLater(final Exception e, final String title,
	...
	public void showCommunicationErrorDialogLater(final Exception e,
			final String message, final Object... args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
	...
	public void showMessageDialog(Component component, Object message,
	...
	public void showMessageDialog(Object message, String title,
			int messageType, Object... args) {
		this.showMessageDialog(this.getFrame(), message, title, messageType,
				args);
	}

	public void showMessageDialog(Component component, Object message,
			String title, int messageType, Object... args) {
		JOptionPane.showMessageDialog(component,
				formatMessage(null, message, args), title, messageType);
	}

	public void showCommunicationErrorDialog(Exception e, String message,
			Object... args) {
		this.showMessageDialog(formatMessage(e, message, args),
	...
	public int showOptionDialog(Object message, String title, int optionType,
		...
		return JOptionPane.showOptionDialog(this.getFrame(), message, title,
				optionType, messageType, icon, options, initialValue);
	}

	@SuppressWarnings({ "StringConcatenationInsideStringBufferAppend" })
	private static String formatMessage(Exception e, Object message,
			Object[] args) {
	...
	public void openLink(String link) {
		...
		try {
			try {
				...
				return;
			} catch (MalformedURLException ignored) { // just means that the
														// link is not a URL
			}

			// It's not a URL, so see if it's a file and invoke the desktop to
			// open it if it is.
			...
			String message = "Error attempting to open resource.\n" + link
					+ (e.getMessage() != null ? "\n" + e.getMessage() : "");
			Util.getLogger().log(Level.SEVERE, message);
			this.showMessageDialog(message, "Error Opening Resource",
					JOptionPane.ERROR_MESSAGE);
		}
	}

	// Raises the Save As dialog to have the user identify the location to save
	// groups of things.
	public File determineSaveLocation(String dialogTitle,
			String defaultFolderName) {
		String defaultPath = this.getFileChooser().getCurrentDirectory()
				.getPath();
		...
		while (true) {
			...
			this.getFileChooser().setFileSelectionMode(
			...
			if (outFile == null) {
				this.showMessageDialog("No location selected", "No Selection",
						JOptionPane.ERROR_MESSAGE);
				continue;
			}
	...
	public File chooseOutputFile(String defaultName, String suffixWithoutDot,
			String dialogTitle) {
		String defaultPath = this.getFileChooser().getCurrentDirectory()
				.getPath();
