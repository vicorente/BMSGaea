...
public class BMS {
	static {
		...
		if (Configuration.isMacOS()) {
			...
			// Autenticamos la app contra el proxy si es necesario
			if (!proxyConfigurationManager.getProperty(
					proxyConfigurationManager.PROXY_USERNAME).equalsIgnoreCase(
					"")) {
				Authenticator
	...
	private static final String APP_CONFIGURATION = "configuration/AppConfiguration.xml";

	public static void main(String[] args) {
		Controller controller = new Controller();

		Dimension appSize = null;
		if (args.length >= 2) // The first two arguments are the application
								// width and height.
			appSize = new Dimension(Integer.parseInt(args[0]),
