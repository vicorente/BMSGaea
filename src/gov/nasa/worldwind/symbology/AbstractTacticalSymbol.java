...
public abstract class AbstractTacticalSymbol extends WWObjectImpl implements
	...
	protected static class IconSource {
		...
		public IconSource(IconRetriever retriever, String symbolId,
				AVList retrieverParams) {
			this.retriever = retriever;
			this.symbolId = symbolId;

			if (retrieverParams != null) {
				// If the specified parameters are non-null, then store a copy
				// of the parameters in this key's params
				// property to insulate it from changes made by the caller. This
				// params list must not change after
		...
		public boolean equals(Object o) {
			...
			if (this.retriever != null ? !this.retriever.equals(that.retriever)
					: that.retriever != null)
				return false;
			if (this.symbolId != null ? !this.symbolId.equals(that.symbolId)
					: that.symbolId != null)
				return false;

			if (this.retrieverParams != null && that.retrieverParams != null) {
				Set<Map.Entry<String, Object>> theseEntries = this.retrieverParams
						.getEntries();
				Set<Map.Entry<String, Object>> thoseEntries = that.retrieverParams
						.getEntries();
		...
		public int hashCode() {
			...
			result = 31 * result
					+ (this.symbolId != null ? this.symbolId.hashCode() : 0);
			result = 31
					* result
					+ (this.retrieverParams != null ? this.retrieverParams
							.getEntries().hashCode() : 0);
			return result;
		}
	...
	// Use an IconKey as the texture's image source. The image source is what
	// defines the contents of this texture,
	...
	protected static class IconTexture extends LazilyLoadedTexture {
		...
		protected TextureData createIconTextureData() {
			try {
				...
				if (image == null) {
					// IconRetriever returns null if the symbol identifier is
					// not recognized, or if the parameter list
					// specified an empty icon. In either case, we mark the
					// texture initialization as having failed to
					// suppress any further requests.
					this.textureInitializationFailed = true;
					return null;
				}

				return AWTTextureIO.newTextureData(
						Configuration.getMaxCompatibleGLProfile(), image,
						this.isUseMipMaps());
			} catch (Exception e) {
				String msg = Logging.getMessage(
						"Symbology.ExceptionRetrievingTacticalIcon",
						this.getImageSource());
				Logging.logger().log(java.util.logging.Level.SEVERE, msg, e);
				this.textureInitializationFailed = true; // Suppress subsequent
															// requests for this
															// tactical icon.
				return null;
			}
		...
		protected static class IconRequestTask implements Runnable {
			...
			protected IconRequestTask(IconTexture texture) {
				if (texture == null) {
					String message = Logging
							.getMessage("nullValue.TextureIsNull");
			...
			public boolean equals(Object o) {
				...
				return this.texture != null ? this.texture.equals(that.texture)
						: that.texture == null;
			}
		...
		protected BufferedImage createModifierImage() {
			try {
				...
				if (image == null) {
					...
					// mark the image initialization as having failed to
					// suppress any further requests.
				...
				String msg = Logging.getMessage(
						"Symbology.ExceptionRetrievingGraphicModifier",
						this.getImageSource());
				Logging.logger().log(java.util.logging.Level.SEVERE, msg, e);
				this.imageInitializationFailed = true; // Suppress subsequent
														// requests for this
														// modifier.
				return null;
			}
	...
	protected static class Label {
		...
		public Label(TextRenderer renderer, String text, Point point,
				Color color) {
			if (renderer == null) {
	...
	public static final String LOADING_IMAGE_PATH = Configuration
			.getStringValue(
					"gov.nasa.worldwind.avkey.MilStd2525LoadingIconPath",
	 ...
	 * space is needed.
	 */
	protected static final TextureAtlas DEFAULT_GLYPH_ATLAS = new TextureAtlas(
			1024, 128, 2048, 2048);
	/**
	...
	static {
		...
		defaultAttrs
				.setTextModifierMaterial(BasicTacticalSymbolAttributes.DEFAULT_TEXT_MODIFIER_MATERIAL);

		// Configure the atlas to remove old texture elements that are likely no
		// longer used to make room for new
	 ...
	 * string identifier during construction, and those specified by calling
	 * {@link #setModifier(String, Object)}. Initialized to a new AVListImpl,
	 * and populated during construction from values in the string identifier
	 * and the modifiers list.
	 */
	 ...
	 * Calculated each frame in
	 * {@link #computeSymbolPoints(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially <code>null</code>.
	 */
	protected Vec4 placePoint;
	/**
	 * Per-frame screen point corresponding to the projection of the placePoint
	 * in the viewport (on the screen). Calculated each frame in
	 * {@link #computeSymbolPoints(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially <code>null</code>.
	 */
	protected Vec4 screenPoint;
	/**
	 * Per-frame distance corresponding to the distance between the placePoint
	 * and the View's eye point. Used to order the symbol as an ordered
	 * renderable, and is returned by getDistanceFromEye. Calculated each frame
	 * in {@link #computeSymbolPoints(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially 0.
	 */
	protected double eyeDistance;
	/**
	 * Per-frame screen scale indicating this symbol's x-scale relative to the
	 * screen offset. Calculated each frame in
	 * {@link #computeTransform(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially 0.
	 */
	protected double sx;
	/**
	 * Per-frame screen scale indicating this symbol's y-scale relative to the
	 * screen offset. Calculated each frame in
	 * {@link #computeTransform(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially 0.
	 */
	...
	/**
	 * Per-frame screen offset indicating this symbol's x-offset relative to the
	 * screenPoint. Calculated each frame in
	 * {@link #computeTransform(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially 0.
	 */
	protected double dx;
	/**
	 * Per-frame screen offset indicating this symbol's y-offset relative to the
	 * screenPoint. Calculated each frame in
	 * {@link #computeTransform(gov.nasa.worldwind.render.DrawContext)}.
	 * Initially 0.
	 */
	 ...
	 * {@link #beginDrawing(gov.nasa.worldwind.render.DrawContext, int)} and
	 * {@link #endDrawing(gov.nasa.worldwind.render.DrawContext)}.
	 */
	protected OGLStackHandler BEogsh = new OGLStackHandler();
	/**
	 * Support for setting up and restoring picking state, and resolving the
	 * picked object. Initialized to a new PickSupport, and used in
	 * {@link #pick(gov.nasa.worldwind.render.DrawContext, java.awt.Point)}.
	 */
	 ...
	 * 
	 * @param position
	 *            the latitude, longitude, and altitude where the symbol is
	 *            drawn.
	 * 
	 * @throws IllegalArgumentException
	 *             if the position is <code>null</code>.
	 */
	...
	public void setAttributes(TacticalSymbolAttributes normalAttrs) {
		this.normalAttrs = normalAttrs; // Null is accepted, and indicates the
	...
	public void setHighlightAttributes(TacticalSymbolAttributes highlightAttrs) {
		this.highlightAttrs = highlightAttrs; // Null is accepted, and indicates
	...
	public void setUnitsFormat(UnitsFormat unitsFormat) {
		...
		// If the unit format is changing then the position needs to be
		// reformatted.
	...
	public void move(Position delta) {
		...
		// The reference position is null if this shape has positions. With
		// PointPlacemark, this should never happen
		// because its position must always be non-null. We check and this case
		// anyway to handle a subclass overriding
		// getReferencePosition and returning null. In this case moving the
		// shape by a relative delta is meaningless
		// because the shape has no geographic location. Therefore we fail
		// softly by exiting and doing nothing.
		if (refPos == null)
			return;
	 ...
	 * See {@link #setOffset(gov.nasa.worldwind.render.Offset) setOffset} for
	 * more information.
	 * 
	 ...
	 * symbol, and other symbols are aligned to the center of the symbol.
	 * {@code setOffset(Offset.CENTER)} aligns the center of the symbol with the
	 * symbol point, and {@code setOffset(Offset.BOTTOM_CENTER)} aligns the
	 * center of the bottom edge with the symbol point.
	 * 
	 * @param offset
	 *            the hot spot controlling the symbol's placement relative to
	...
	protected String getFormattedPosition() {
		...
		// Format the position to a string only when necessary.
		// formattedPosition is set to null when either the
	...
	protected void setDepthOffset(Double depthOffset) {
		this.depthOffset = depthOffset; // Null is accepted, and indicates the
										// default depth offset is used.
	}
	...
	protected void setGlyphAtlas(TextureAtlas atlas) {
		// Note that we do not explicitly remove this symbol's glyphs from the
		// old atlas. The modifier texture atlas
		// should be configured to evict the oldest glyphs when the atlas is
		// full. Leaving this symbol's glyphs in the
		// atlas does not incur any additional overhead, and has the benefit of
		// ensuring that we do not remove glyphs
	...
	protected void makeOrderedRenderable(DrawContext dc) {
		// Calculate this symbol's per-frame values, re-using values already
		// calculated this frame.
		if (dc.getFrameTimeStamp() != this.frameNumber) {
			// Compute the model and screen coordinate points corresponding to
			// the position and altitude mode.
			...
			// If the symbol has never been laid out perform a frustum test
			// using estimated screen bounds. If the symbol
			// is not visible, then don't compute layout. This avoids
			// downloading icons and laying out symbols that are
			// not yet visible.
			if (this.screenRect == null && !this.intersectsFrustum(dc))
				return;

			// Compute the currently active attributes from either the normal or
			// the highlight attributes.
			this.determineActiveAttributes();
			if (this.getActiveAttributes() == null)
				return;

			// Compute the scale for this frame. This must happen before layout
			// because the text layout may depend
			// on the scale.
			this.computeScale();
			...
			// Compute the offset parameters that are applied during rendering.
			// This must be done after
			// layout, because the transform depends on the frame rectangle
	...
	protected void computeSymbolPoints(DrawContext dc) {
		...
		if (this.altitudeMode == WorldWind.CLAMP_TO_GROUND) {
			this.placePoint = dc.computeTerrainPoint(pos.getLatitude(),
					pos.getLongitude(), 0);
		...
		} else // Default to ABSOLUTE
		{
			...
			this.placePoint = dc.getGlobe().computePointFromPosition(
					pos.getLatitude(), pos.getLongitude(), height);
		}

		if (this.placePoint == null)
			return;

		// Compute the symbol's screen location the distance between the eye
		// point and the place point.
		this.screenPoint = dc.getView().project(this.placePoint);
		this.eyeDistance = this.placePoint.distanceTo3(dc.getView()
				.getEyePoint());
	}
	...
	protected void determineActiveAttributes() {
		...
		if (this.isHighlighted()) {
			if (this.getHighlightAttributes() != null)
				...
				// either the normal or default attributes
		...
		// If the font has changed since the last frame, then the layout needs
		// to be recomputed since text may be a
		// different size.
		...
		// Opacity does not directly affect layout, but each label stores the
		// opacity in its material. If the opacity
	...
	protected void layout(DrawContext dc) {
		...
		boolean mustDrawModifiers = this.mustDrawGraphicModifiers(dc)
				|| this.mustDrawTextModifiers(dc);

		// If the icon retrieval parameters have changed then the icon needs to
		// be updated, which may affect layout.
		AVList retrieverParams = this.assembleIconRetrieverParameters(null);
		IconSource iconSource = new IconSource(this.getIconRetriever(),
				this.getIdentifier(), retrieverParams);
		...
		if (this.mustLayout(iconSource, modifierParams)) {
			...
			// Set the unresolved flag false. addGlyph will set it to true if
			// there are still unresolved resources.
		...
		// Layout dynamic modifiers each frame because they are expected to
		// change each frame.
		if (mustDrawModifiers)
	...
	protected boolean mustLayout(IconSource iconSource, AVList modifiers) {
		// If one or more glyphs need to be resolved, then layout is not
		// complete.
		...
		// If the modifiers have changed since layout was computed then it needs
		// to be recomputed.
		...
		if (this.iconTexture == null
				|| this.iconTexture != this.activeIconTexture)
			return true;
	...
	protected void layoutIcon(DrawContext dc, IconSource source) {
		...
		// Lazily create the symbol icon texture when either the IconRetriever,
		// the symbol ID, or the retriever
		// parameters change.
		if (this.iconTexture == null
				|| !this.iconTexture.getImageSource().equals(source))
			this.iconTexture = new IconTexture(source);

		// Use the currently active icon texture until the new icon texture (if
		// any) has successfully loaded. This
		// ensures that the old icon texture continues to display until the new
		// icon texture is ready, and avoids
		// temporarily displaying nothing.
		if (this.activeIconTexture != this.iconTexture
				&& this.iconTexture != null && this.iconTexture.bind(dc)) {
			this.activeIconTexture = this.iconTexture;
			this.iconRect = null; // Recompute the icon rectangle when the
									// active icon texture changes.
		}

		// If the icon is not available then draw a default icon. Only draw the
		// default before any icon is loaded. If
		// the icon is changed after loading then we will continue to draw the
		// old icon until the new one becomes
		...
		// Lazily compute the symbol icon rectangle only when necessary, and
		// only after the symbol icon texture has
		// successfully loaded.
		if (this.iconRect == null && textureLoaded) {
			// Compute the symbol icon's frame rectangle in local coordinates.
			// This is used by the modifier layout to
			// determine where to place modifier graphics and modifier text.
			// Note that we bind the texture in order to
			...
			Point2D point = this.iconOffset != null ? this.iconOffset
					.computeOffset(w, h, null, null) : new Point(0, 0);
			Dimension size = this.iconSize != null ? this.iconSize.compute(w,
					h, w, h) : new Dimension(w, h);
			this.iconRect = new Rectangle((int) point.getX(),
					(int) point.getY(), size.width, size.height);
		}

		// Add the symbol icon rectangle to the screen rectangle and layout
	...
	protected AVList assembleIconRetrieverParameters(AVList params) {
		...
		Material interiorMaterial = this.getActiveAttributes()
				.getInteriorMaterial();
	 ...
	 * this method. Instead, subclasses may override
	 * {@link #layoutGraphicModifiers(gov.nasa.worldwind.render.DrawContext, gov.nasa.worldwind.avlist.AVList)
	 * layoutGraphicModifiers} and
	 * {@link #layoutTextModifiers(gov.nasa.worldwind.render.DrawContext, gov.nasa.worldwind.avlist.AVList)
	 * layoutTextModifiers}.
	 * 
	 * @param dc
	...
	protected void layoutStaticModifiers(DrawContext dc, AVList modifiers) {
		...
		// Compute the bounds of the symbol and graphic modifiers with scaling
		// applied. The text will be laid out
	...
	protected void layoutGraphicModifiers(DrawContext dc, AVList modifiers) {
		// Intentionally left blank. Subclasses can override this method in
		// order to layout any modifiers associated
	 ...
	 * 
	 * @param modifiers
	 *            List of modifiers. This method may modify this list by adding
	 *            implicit modifiers.
	 */
	protected void applyImplicitModifiers(AVList modifiers) {
		// Intentionally left blank. Subclasses can override this method in
		// order to add modifiers that are implicitly
	 ...
	 * 
	 * @param offset
	 *            Offset into either the {@code iconRect} or {@code layoutRect}
	 *            at which to align the hot spot.
	 * @param hotspot
	 *            Offset into the rectangle of the hot spot.
	 * @param size
	 *            Size of the rectangle.
	 * @param layoutMode
	 *            One of {@link #LAYOUT_ABSOLUTE}, {@link #LAYOUT_RELATIVE}, or
	 *            {@link #LAYOUT_NONE}.
	 * 
	 * @return the laid out rectangle.
	 */
	protected Rectangle layoutRect(Offset offset, Offset hotspot,
			Dimension size, Object layoutMode) {
		int x = 0;
		int y = 0;
		...
		if (offset != null) {
			...
			Point2D p = offset.computeOffset(rect.getWidth(), rect.getHeight(),
					null, null);
			x += rect.getX() + p.getX();
			y += rect.getY() + p.getY();
		}
		...
		if (hotspot != null) {
			Point2D p = hotspot.computeOffset(size.getWidth(),
					size.getHeight(), null, null);
			x -= p.getX();
			y -= p.getY();
		}
		...
		if (LAYOUT_ABSOLUTE.equals(layoutMode)
				|| LAYOUT_RELATIVE.equals(layoutMode)) {
			if (this.layoutRect != null)
	 ...
	 * @return the laid out rectangle.
	 */
	protected Rectangle layoutLabelRect(Offset offset, Offset hotspot,
			Dimension size, Object layoutMode) {
		int x = 0;
		int y = 0;
		...
		if (LAYOUT_ABSOLUTE.equals(layoutMode)
			...
			// Compute where the label rectangle falls in the icon layout before
			// scaling is applied. This is necessary
			// to layout graphic modifiers such as the ground direction of
			// movement indicator that are scaled down with
			// the icon, but should not overlap text which is not scaled with
			// the icon.
			Rectangle scaledRect = this.computeScaledRect(rect, rect.getSize(),
					1 / this.sx, 1 / this.sy);
			if (this.layoutRect != null)
	...
	protected List<? extends Point2D> layoutPoints(Offset offset,
			List<? extends Point2D> points, Object layoutMode,
		...
		for (int i = 0; i < points.size(); i++) {
			...
			if (this.screenRect != null)
				...
				this.screenRect = new Rectangle((int) p.getX(), (int) p.getY(),
						0, 0);

			if (i < numPointsInLayout
					&& (LAYOUT_ABSOLUTE.equals(layoutMode) || LAYOUT_RELATIVE
							.equals(layoutMode))) {
				if (this.layoutRect != null)
					this.layoutRect.add(p);
				else
					this.layoutRect = new Rectangle((int) p.getX(),
							(int) p.getY(), 0, 0);
			}
		}
	...
	protected void addGlyph(DrawContext dc, Offset offset, Offset hotspot,
			String modifierCode) {
		this.addGlyph(dc, offset, hotspot, modifierCode, null, null);
	}

	protected void addGlyph(DrawContext dc, Offset offset, Offset hotspot,
			String modifierCode, AVList retrieverParams, Object layoutMode) {
		IconAtlasElement elem = this.getGlyph(modifierCode, retrieverParams);

		if (elem.load(dc)) {
			Rectangle rect = this.layoutRect(offset, hotspot, elem.getSize(),
					layoutMode);
	...
	protected void addLabel(DrawContext dc, Offset offset, Offset hotspot,
			String modifierText) {
		this.addLabel(dc, offset, hotspot, modifierText, null, null, null);
	}

	protected void addLabel(DrawContext dc, Offset offset, Offset hotspot,
			String modifierText, Font font, Color color, Object layoutMode) {
		if (font == null) {
			// Use either the currently specified text modifier font or compute
			// a default if no font is specified.
		...
		if (color == null) {
			// Use either the currently specified text modifier material or the
			// default if no material is specified.
			Material material = this.getActiveAttributes()
					.getTextModifierMaterial();
			if (material == null)
				material = BasicTacticalSymbolAttributes.DEFAULT_TEXT_MODIFIER_MATERIAL;

			// Use either the currently specified opacity or the default if no
			...
			color = new Color(diffuse.getRed(), diffuse.getGreen(),
					diffuse.getBlue(), alpha);
		}

		TextRenderer tr = OGLTextRenderer.getOrCreateTextRenderer(
				dc.getTextRendererCache(), font);
		Rectangle bounds = tr.getBounds(modifierText).getBounds();
		Rectangle rect = this.layoutLabelRect(offset, hotspot,
				bounds.getSize(), layoutMode);
		Point point = new Point(rect.getLocation().x, rect.getLocation().y
				+ bounds.y + bounds.height);

		this.currentLabels.add(new Label(tr, modifierText, point, color));
	}

	protected void addLine(DrawContext dc, Offset offset,
	...
	protected void addLine(DrawContext dc, Offset offset,
			List<? extends Point2D> points, Object layoutMode,
			int numPointsInLayout) {
		points = this.layoutPoints(offset, points, layoutMode,
				numPointsInLayout);
		this.currentLines.add(new Line(points));
	}

	protected IconAtlasElement getGlyph(String modifierCode,
			AVList retrieverParams) {
		...
		if (elem == null) {
			IconSource source = new IconSource(this.getModifierRetriever(),
					modifierCode, retrieverParams);
	...
	protected void removeDeadModifiers(long now) {
		...
		List<String> deadKeys = null; // Lazily created below to avoid
										// unnecessary allocation.

		for (Map.Entry<String, IconAtlasElement> entry : this.glyphMap
				.entrySet()) {
	...
	protected void computeTransform(DrawContext dc) {
		if (this.getOffset() != null && this.iconRect != null) {
			Point2D p = this.getOffset().computeOffset(
					this.iconRect.getWidth(), this.iconRect.getHeight(), null,
					null);
	...
	/**
	 * Compute the bounds of symbol after the scale has been applied. This
	 * method computes {@link #iconRectScaled} and {@link #layoutRectScaled}.
	 * 
	 * @param dc
	 *            Current draw context.
	 * @param modifiers
	...
	protected void computeScaledBounds(DrawContext dc, AVList modifiers) {
		...
		this.iconRectScaled = this.computeScaledRect(this.iconRect,
				maxDimension, this.sx, this.sy);
		this.layoutRectScaled = this.computeScaledRect(this.layoutRect,
				maxDimension, this.sx, this.sy);
	}
	...
	protected Dimension computeMinTextLayout(DrawContext dc, AVList modifiers) {
		// Use either the currently specified text modifier font or compute a
		// default if no font is specified.
		...
		// Get the bounds of "E" to estimate how tall a typical line of text
		// will be.
		Rectangle2D bounds = tr.getBounds("E");

		// Determine how many lines of text to expect so that we can compute a
		// reasonable minimum size.
		int maxLines = this.getMaxLabelLines(modifiers);

		int maxDim = (int) (bounds.getHeight() * maxLines * 1.5); // Add 50% for
																	// line
																	// spacing
	...
	protected Rectangle computeScaledRect(Rectangle rect,
			Dimension maxDimension, double scaleX, double scaleY) {
		...
		return new Rectangle((int) x, (int) y, (int) Math.ceil(width),
	...
	public Rectangle computeScreenExtent() {
		...
		if (this.screenRect != null) {
			x = this.screenPoint.x + this.sx
					* (this.dx + this.screenRect.getX());
			y = this.screenPoint.y + this.sy
					* (this.dy + this.screenRect.getY());
	...
	protected boolean intersectsFrustum(DrawContext dc) {
		...
		// Test the symbol's model coordinate point against the near and far
		// clipping planes.
		if (this.placePoint != null
				&& (view.getFrustumInModelCoordinates().getNear()
						.distanceTo(this.placePoint) < 0 || view
						.getFrustumInModelCoordinates().getFar()
						.distanceTo(this.placePoint) < 0)) {
			return false;
		}
	...
	protected void drawBatched(DrawContext dc) {
		...
		if (!dc.isPickingMode()) {
			while (nextItem != null
					&& nextItem instanceof AbstractTacticalSymbol) {
			...
			while (nextItem != null
				...
				if (ts.pickLayer != this.pickLayer) // batch pick only within a
													// single layer
					break;
	...
	protected void beginDrawing(DrawContext dc, int attrMask) {
		...
		attrMask |= GL2.GL_DEPTH_BUFFER_BIT // for depth test enable, depth
											// func, depth mask
				| GL2.GL_COLOR_BUFFER_BIT // for alpha test enable, alpha func,
											// blend enable, blend func
		...
		// Enable OpenGL vertex arrays for all symbols by default. All tactical
		// symbol drawing code specifies its data
		// to OpenGL using vertex arrays.
		gl.glEnableClientState(GL2.GL_VERTEX_ARRAY);
		gl.glEnableClientState(GL2.GL_TEXTURE_COORD_ARRAY);

		// Enable the alpha test to suppress any fully transparent image pixels.
		// We do this for both normal rendering
		// and picking because it eliminates fully transparent texture data from
		// contributing to the pick frame.
		gl.glEnable(GL2.GL_ALPHA_TEST);
		gl.glAlphaFunc(GL2.GL_GREATER, 0f);

		// Apply the depth buffer but don't change it (for screen-space
		// symbols).
		...
		// Enable OpenGL texturing for all symbols by default. The most common
		// case is to render a series of textured
		...
		if (dc.isPickingMode()) {
			...
			// Give symbol modifier lines a thicker width during picking in
			// order to make them easier to select.
			gl.glLineWidth(9f);
		} else {
			// Enable blending for RGB colors which have been premultiplied by
			// their alpha component. We use this mode
			// because the icon texture and modifier textures RGB color
			// components have been premultiplied by their color
			// component.
			gl.glEnable(GL.GL_BLEND);
			OGLUtil.applyBlending(gl, true);

			// Give symbol modifier lines a 3 pixel wide anti-aliased
			// appearance. This GL state does not affect the
	...
	protected void endDrawing(DrawContext dc) {
		...
		if (dc.isPickingMode()) {
			...
			gl.glTexEnvi(GL2.GL_TEXTURE_ENV, GL2.GL_SRC0_RGB,
					OGLUtil.DEFAULT_SRC0_RGB);
			gl.glTexEnvi(GL2.GL_TEXTURE_ENV, GL2.GL_COMBINE_RGB,
	...
	protected void doDrawOrderedRenderable(DrawContext dc,
		...
		if (dc.isPickingMode()) {
			...
			pickCandidates.addPickableObject(this.createPickedObject(pickColor
					.getRGB()));
			gl.glColor3ub((byte) pickColor.getRed(),
					(byte) pickColor.getGreen(), (byte) pickColor.getBlue());
		} else {
			// Set the current color to white with the symbol's current opacity.
			// This applies the symbol's opacity to
			// its icon texture and graphic modifier textures by multiplying
			// texture fragment colors by the opacity. We
			// pre-multiply the white RGB color components by the alpha since
			// the texture's RGB color components have
			// also been pre-multiplied by their color component.
			float a = this.getActiveAttributes().getOpacity() != null ? this
					.getActiveAttributes().getOpacity().floatValue()
		...
		try {
			// Apply any custom depth offset specified by the caller. This
			// overrides the default depth offset specified
			// in beginRendering, and is therefore restored in the finally block
			// below.
			...
			// If the caller specified a custom depth offset, we restore the
			// default depth offset to the value specified
			// in beginRendering.
			if (depthOffsetUnits != null)
	...
	protected void prepareToDraw(DrawContext dc) {
		// Apply the symbol's offset in screen coordinates. We translate the X
		// and Y coordinates so that the
		// symbol's hot spot (identified by its offset) is aligned with its
		// screen point. We translate the Z
		// coordinate so that the symbol's depth values are appropriately
		// computed by OpenGL according to its
		// distance from the eye. The orthographic projection matrix configured
		// in beginRendering correctly maps
		// the screen point's Z coordinate to its corresponding depth value.
		GL2 gl = dc.getGL().getGL2(); // GL initialization checks for GL2
										// compatibility.
		gl.glLoadIdentity(); // Assumes that the current matrix mode is
								// GL_MODELVIEW.
		gl.glTranslated(this.screenPoint.x, this.screenPoint.y,
				this.screenPoint.z);
	}
	...
	protected void draw(DrawContext dc) {
		...
		if (this.mustDrawTextModifiers(dc) && !dc.isPickingMode()) {
			try {
				// Do not apply scale to text modifiers. The size of the text is
				// determined by the font. Do apply scale
	...
	protected void drawIcon(DrawContext dc) {
		...
		try {
			gl.glPushMatrix();
			gl.glScaled(this.activeIconTexture.getWidth(dc),
					this.activeIconTexture.getHeight(dc), 1d);
	...
	protected void drawLabels(DrawContext dc) {
		...
		try {
			// Don't depth buffer labels. Depth buffering would cause the labels
			// to intersect terrain, which is
			// usually a bigger usability problem for text than a label showing
			// through a hill.
	...
	protected void drawLines(DrawContext dc) {
		// Use either the currently specified opacity or the default if no
		// opacity is specified.
		Double opacity = this.getActiveAttributes().getOpacity() != null ? this
				.getActiveAttributes().getOpacity()
		...
		try {
			...
			// Set the current color to black with the current opacity value as
			// the alpha component. Blending is set to
			// pre-multiplied alpha mode, but we can just specify 0 for the RGB
			// components because multiplying them by
			...
			// Restore the depth range and texture 2D enable state to the values
			// specified in beginDrawing.
			gl.glEnable(GL.GL_TEXTURE_2D);
			gl.glDepthRange(0.0, 1.0);

			// Restore the current color to that specified in
			// doDrawOrderedRenderable.
			if (!dc.isPickingMode())
				gl.glColor4f(opacity.floatValue(), opacity.floatValue(),
						opacity.floatValue(), opacity.floatValue());
		}
	}
