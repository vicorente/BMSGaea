 ...
 */
public class ScrollFrame extends DragControl implements PreRenderable,
		Renderable {
	...
	protected static final Size DEFAULT_MAXIMIZED_SIZE = Size.fromPixels(265,
			300);
	...
	protected FrameAttributes activeAttributes = new BasicFrameAttributes(); // re-determined
																				// each
																				// frame
	protected static Offset DEFAULT_OFFSET=null;
	/**
	...
	// Frame title fields. These fields are recomputed when the frame size
	// changes, the title text changes, or the
	// title font changes.
	/**
	 ...
	 * 
	 * @param x
	 *            x coordinate of the upper left corner of the frame, in AWT
	 *            screen coordinates (origin upper left corner of the screen).
	 * @param y
	 *            y coordinate of the upper left corner of the frame, in AWT
	 *            screen coordinates (origin upper left corner of the screen).
	 */
	public ScrollFrame(int x, int y) {
	
		this(DEFAULT_OFFSET = new Offset((double) x, (double) y, AVKey.PIXELS,
				AVKey.INSET_PIXELS));
	}
	 ...
	 *            initial location of the upper left corner of the frame.
	 */
	 ...
	 * 
	 * @param minimized
	 *            {@code true} if the frame must be minimized. {@code false} if
	 *            the frame must not be minimized.
	 */
	 ...
	 * 
	 * @param highlighted
	 *            {@code true} if the frame is now highlighted.
	 */
	...
	public void setFrameTitle(String frameTitle) {
		...
		// Invalidate the computed title for display. It will be regenerated the
		// next time the frame is rendered.
	 ...
	 *         there is no minimized size, in which case the normal maximized
	 *         frame size is used in the minimized state.
	 */
	 ...
	 * 
	 * @param size
	 *            the size of the minimized frame. Set {@code null} to use the
	 ...
	 * 
	 * @param size
	 *            the maximum size of the minimized frame. Set {@code null} for
	 *            no maximum.
	 */
	...
	public void setTitleBarHeight(int titleBarHeight) {
		if (titleBarHeight < 0) {
			String message = Logging.getMessage("generic.InvalidHeight",
					titleBarHeight);
			Logging.logger().severe(message);
			throw new IllegalArgumentException(message);
		}
	 ...
	 * 
	 * @param enable
	 *            {@code true} to allow the user to resize the frame by dragging
	 *            the border.
	 */
	 ...
	 * 
	 * @param enable
	 *            {@code true} if the user is allowed to move the frame by
	 *            dragging.
	 */
	 ...
	 * 
	 * @param minimizeAnimation
	 *            New minimize animation.
	 * 
	 ...
	 * 
	 * @param imageSource
	 *            New image source. May be a String, URL, or BufferedImage.
	 */
	...
	public Rectangle getBounds(DrawContext dc) {
		...
		return new Rectangle((int) this.awtScreenPoint.getX(),
				(int) this.awtScreenPoint.getY(), this.frameSize.width,
	 ...
	 * 
	 * @param attributes
	 *            new attributes bundle for normal state.
	 */
	 ...
	 * 
	 * @param attributes
	 *            new attributes bundle for highlight state.
	 */
	 ...
	 * 
	 * @param rows
	 *            Number of rows of tiles in the contents.
	 * @param columns
	...
	protected boolean mustUpdateTiles(DrawContext dc) {
		// Tiles are not visible if the frame is minimized, so no reason to
		// update
		if (this.isMinimized())
			return false;

		// Make sure that our texture is available. If the texture has been
		// evicted it will need to be regenerated.
	 ...
	 * @param tile
	 *            Tile to test.
	 * @param contentUpdateTime
	 *            Time at which the content was last updated.
	 * 
	 * @return {@code true} if the tile needs to be updated. Always returns
	 *         {@code false} if the tile is not visible in the current frame
	 *         bounds.
	 */
	...
	protected void updateTiles(DrawContext dc) {
		// The OpenGL framebuffer object extension used by
		// RenderToTextureSupport works only for texture formats
		// GL_RGB and GL_RGBA.
		this.rttSupport.setEnableFramebufferObject(true);

		Texture texture = dc.getTextureCache().getTexture(this.textureCacheKey);

		// Determine how large of a texture is required to render the full frame
		// bounds.
		int dim = this.computeTileTextureDimension(
				this.contentBounds.getSize(), this.contentSize);

		// Check the texture dimension against the maximum supported by the
		// hardware
		int maxTexture = dc.getGLRuntimeCapabilities().getMaxTextureSize();
		this.renderToTexture = (dim <= maxTexture);

		// If the frame is too big to render into a texture don't bother
		// building tiles
		if (!this.renderToTexture) {
			return;
		}

		// If we don't have a texture, or if we need a different size of
		// texture, allocate a new one
		if (texture == null || this.textureDimension != dim) {
			...
			// Create entries for the sub-tiles in the texture. Each sub-tile
			// will be used to render a piece of the
			// frame contents.
			...
			// Clear the texture tile map. Any previous tile allocations are now
			// invalid.
		...
		if (texture == null) // This should never happen, but we check anyway.
		{
			Logging.logger().warning(
					Logging.getMessage("nullValue.TextureIsNull"));
			return;
		}

		int rows = (int) Math.ceil((double) this.contentSize.height
				/ this.textureTileDimension);
		int columns = (int) Math.ceil((double) this.contentSize.width
				/ this.textureTileDimension);
		...
		for (ContentTile tile : this.tiles) {
			if (this.mustUpdateTile(tile, contentUpdateTime)) {
				...
				Rectangle tileBounds = new Rectangle(x, y,
						this.textureTileDimension, this.textureTileDimension);

				this.rttSupport.beginRendering(dc, tileBounds.x, tileBounds.y,
	 ...
	 * 
	 * @param tile
	 *            ContentTile for which to get a texture tile.
	 * 
	 ...
	 * 
	 * @param tile
	 *            ScrollableTile for which to allocate a texture tile.
	 * 
	 * @return TextureTile allocated for the ScrollableTile.
	 */
	protected TextureTile allocateTextureTile(ContentTile tile) {
		// Sort the list of texture tiles so that we can find the one that is
		// the least recently used.
		TextureTile[] timeOrderedEntries = new TextureTile[this.textureTiles
				.size()];
	 ...
	 * 
	 * @param dc
	 *            the draw context the tile relates to.
	 * @param tile
	 *            the tile to update. A new texture tile will be allocated for
	 *            the tile, if the tile does not have a texture.
	 * @param tileBounds
	 *            bounds of the tile being updated, within the larger texture.
	 */
	protected void updateTile(DrawContext dc, ContentTile tile,
			Rectangle tileBounds) {
		int x = tileBounds.x - tile.column * this.textureTileDimension;
		int y = tileBounds.y - this.contentSize.height
				+ this.textureTileDimension * (tile.row + 1);
		Rectangle scrollBounds = new Rectangle(x, y, this.contentBounds.width,
				this.textureTileDimension);

		try {
			Texture texture = dc.getTextureCache().getTexture(
					this.textureCacheKey);

			this.rttSupport.setColorTarget(dc, texture);
			this.rttSupport.clear(dc, new Color(0, 0, 0, 0)); // Set all texture
																// pixels to
																// transparent
																// black.

			this.contents.renderScrollable(dc, scrollBounds.getLocation(),
					scrollBounds.getSize(), tileBounds);
		} finally {
	...
	protected Texture createTileTexture(DrawContext dc, int width, int height) {
		...
		TextureData td = new TextureData(gl.getGLProfile(), // GL profile
				...
				null, null) // buffer, flusher
		{
			...
			public int getEstimatedMemorySize() {
				int sizeInBytes = OGLUtil.estimateTextureMemorySize(
						this.getInternalFormat(), this.getWidth(),
	...
	protected int computeTileTextureDimension(Dimension frameSize,
		...
		return WWMath.powerOfTwoCeiling((int) Math.sqrt(area)
				+ this.textureTileDimension);
	}
	...
	public void preRender(DrawContext dc) {
		...
		// Si la pantalla se maximiza, todos los elementos vuelven a su posición
		// original
		GraphicsEnvironment env = GraphicsEnvironment
				.getLocalGraphicsEnvironment();	

		if (dc.getView().getViewport().getBounds().getMaxX() == env
				.getMaximumWindowBounds().getMaxX()) {
			//this.setSize(new Size());
			//this.setScreenLocation(DEFAULT_OFFSET);
		}

		if (dc.getView().getViewport().getHeight() > 200
			...
			if (this.contentBounds.y < 0.0) {
				screenLocation.setY(dc.getView().getViewport().getHeight()
						- this.contentBounds.height - 35);
			}
			...
			if (dc.getView().getViewport().getWidth() < this.getSize()
				...
				tamanio.setWidth(
						gov.nasa.worldwind.render.Size.EXPLICIT_DIMENSION, dc
								.getView().getViewport().getWidth(),
						AVKey.PIXELS);
			...
			if (dc.getView().getViewport().getHeight() < this.getSize()
				...
				tamanio.setHeight(
						gov.nasa.worldwind.render.Size.EXPLICIT_DIMENSION, dc
								.getView().getViewport().getHeight(),
						AVKey.PIXELS);
				this.setSize(tamanio);
			}
		}
		// Highlight the frame if the pick point is within the frame's pickable
		// bounds.
		Point pickPoint = dc.getPickPoint();
		if (pickPoint != null) {
			int glY = dc.getView().getViewport().height - pickPoint.y;
			this.setHighlighted(this.pickBounds.contains(new Point(pickPoint.x,
					glY)));
		}
	...
	public void render(DrawContext dc) {
		...
		if (this.intersectsFrustum(dc)) {
			try {
				...
				// While the tree is animated toward a minimized state, draw it
				// as if it were maximized,
		...
		this.minimizeButton = new TreeHotSpot(this) {
			...
			public void selected(SelectEvent event) {
				...
				if (event.isLeftClick()) {
					ScrollFrame.this.setMinimized(!ScrollFrame.this
							.isMinimized());
					event.consume();
				} else {
	 ...
	 * 
	 * @param row
	 *            Row of the tile to get the bounds of.
	 * @param column
	 *            Column of the tile to get the bounds of.
	 * 
	...
	protected Rectangle getContentTileBounds(int row, int column) {
		...
		int tileScreenX = (int) this.contentBounds.getMinX()
				+ this.textureTileDimension * column - xScroll;
		int tileScreenY = (int) this.contentBounds.getMaxY()
				- this.textureTileDimension * (row + 1) + yScroll;

		return new Rectangle(tileScreenX, tileScreenY,
				this.textureTileDimension, this.textureTileDimension);
	}
	...
	public void updateBounds(DrawContext dc) {
		...
		// If the frame size is relative to the content size, compute the
		// content size and then set the frame size.
		if (this.isRelativeSize(size)) {
			// Pass null for the frame bounds because the frame size depends on
			// the content size.
			contentSize = this.contents.getSize(dc, null);
			Dimension frameSizeForContentSize = this
					.computeFrameRectForContentRect(contentSize);

			this.frameSize = size.compute(frameSizeForContentSize.width,
					frameSizeForContentSize.height, viewport.width,
					viewport.height);
		} else {
			// Otherwise just compute the frame size. The content size will be
			// computed after the frame size has been
			// determined.
			this.frameSize = size
					.compute(0, 0, viewport.width, viewport.height);
		}

		// Apply the maximum size constraint
		if (this.getMaxSize() != null) {
			Dimension max = this.getMaxSize().compute(this.frameSize.width,
					this.frameSize.height, viewport.width, viewport.height);
		...
		Point2D upperLeft = this.screenLocation.computeOffset(viewport.width,
				viewport.height, 1.0, 1.0);

		this.awtScreenPoint = new Point((int) upperLeft.getX(),
				(int) (viewport.height - upperLeft.getY()));

		this.frameBounds = new Rectangle((int) upperLeft.getX(),
				(int) upperLeft.getY() - this.frameSize.height,
				this.frameSize.width, this.frameSize.height);

		// Compute the pickable screen extent as the frame extent, plus the
		// width of the frame's pickable outline.
		// This extent is used during picking to ensure that the frame's outline
		// is pickable when it exceeds the
		// frame's screen extent.
		this.pickBounds = new Rectangle(this.frameBounds.x
				- this.borderPickWidth / 2, this.frameBounds.y
				- this.borderPickWidth / 2, this.frameBounds.width
				+ this.borderPickWidth, this.frameBounds.height
				+ this.borderPickWidth);

		this.innerBounds = new Rectangle((int) upperLeft.getX()
				+ this.frameBorder, (int) upperLeft.getY() - frameSize.height
				+ this.frameBorder, frameSize.width - this.frameBorder * 2,
				frameSize.height - this.frameBorder * 2);

		// If the content size has yet not been computed, compute it now.
		if (contentSize == null) {
			// Compute the bounds as if both scroll bars are visible. This saves
			// us from having to compute the size
			// multiple times if scroll bars are required. If scroll bars are
			// not required it may leave a little bit of
			// extra padding on the edges of the frame.
			contentSize = this.contents.getSize(dc,
					this.computeBounds(true, true).getSize());
		}

		// Computing the bounds of the content area of the frame requires
		// computing the bounds with no scroll bars,
		// and determining if scroll bars are required given the size of the
		// scrollable content.
		...
		boolean showVerticalScrollbar = this
				.mustShowVerticalScrollbar(contentSize);

		// If we need a vertical scroll bar, recompute the bounds because the
		// scrollbar consumes horizontal space
		if (showVerticalScrollbar) {
			this.contentBounds = this.computeBounds(true, false);
		}

		// Determine if we need a horizontal scroll bar
		boolean showHorizontalScrollbar = this
				.mustShowHorizontalScrollbar(contentSize);

		// If we need a horizontal scroll bar, recompute the bounds because the
		// horizontal scroll bar consumes vertical
		// space and a vertical scroll bar may now be required
		if (showHorizontalScrollbar && !showVerticalScrollbar) {
			this.contentBounds = this.computeBounds(showVerticalScrollbar,
		...
		// If the scroll bars were visible and are now hidden, reset the scroll
		// position to zero. Otherwise, the
		// scroll bar will already be scrolled to a certain position when it
		// reappears, which is probably not what
		// the user expects.
		...
		this.scrollContentBounds.y = this.scrollContentBounds.y
				- (contentSize.height - this.contentBounds.height);
	...
	protected Dimension computeFrameRectForContentRect(Dimension contentSize) {
		int frameWidth = contentSize.width + this.frameBorder * 2 + 4
				* this.frameLineWidth + this.scrollBarSize;
		int frameHeight = contentSize.height + this.frameBorder * 2
				+ this.getTitleBarHeight() + 2 * this.frameLineWidth;
	 ...
	 *         size of the frame content.
	 */
	// TODO try to eliminate this dependence on size modes. This would break if
	// an app subclassed Size and implemented
	 ...
	 * 
	 * @param contentSize
	 *            The total size, in pixels, of the scrollable content.
	 * 
	...
	protected boolean mustShowVerticalScrollbar(Dimension contentSize) {
		// If the frame is not minimized or in the middle of an animation,
		// compare the content size to the visible
		// bounds.
		if ((!this.isMinimized() && !this.isAnimating())) {
			return contentSize.height > this.contentBounds.height;
		} else {
			// Otherwise, return the previous scrollbar setting, do not
			// recompute it. While the frame is animating, we want
			// the scrollbar decision to be based on its maximized size. If the
			// frame would have scrollbars when maximized will
			// have scrollbars while it animates, but a frame that would not
			// have scrollbars when maximized will not have
	...
	protected boolean mustShowHorizontalScrollbar(Dimension contentSize) {
		// Show a scroll bar if the content is large enough to require a scroll
		// bar, and there is enough space to
		// draw the scroll bar.
		return contentSize.width > this.contentBounds.width
				&& this.innerBounds.height > this.titleBarHeight
						+ this.scrollBarSize;
	}
	 ...
	 * 
	 * @param showVerticalScrollBar
	 *            True if the frame will have a vertical scroll bar. A vertical
	 *            scroll bar will make the content frame narrower.
	 * @param showHorizontalScrollBar
	 *            True if the frame will have a horizontal scroll bar. A
	 *            horizontal scroll bar will make the content frame shorter.
	 * 
	...
	protected Rectangle computeBounds(boolean showVerticalScrollBar,
		...
		return new Rectangle(this.innerBounds.x + inset, this.innerBounds.y
				+ hScrollBarSize + inset, this.innerBounds.width
				- vScrollBarSize - inset * 2, this.innerBounds.height
				- titleBarHeight - hScrollBarSize - inset);
	}
	...
	protected void computeFrameGeometry() {
		...
		this.vertexBuffer = FrameFactory.createShapeBuffer(
				AVKey.SHAPE_RECTANGLE, this.frameBounds.width,
				this.frameBounds.height, attributes.getCornerRadius(),
				this.vertexBuffer);
	}
	...
	protected Dimension getMinimumSize() {
		// Reserve enough space to draw the border, both scroll bars, and the
		// title bar
		int minWidth = this.frameBorder * 2 + this.scrollBarSize * 3; // left
																		// scroll
																		// arrow
																		// +
																		// right
		...
		int minHeight = this.frameBorder * 2 + this.scrollBarSize * 3
				+ this.titleBarHeight; // Up arrow + down arrow + horizontal
										// scroll bar
	...
	protected boolean isDrawMinimized() {
		// Draw minimized when the frame is minimized, but not while animating
		// toward the minimized state
	...
	protected void drawMaximized(DrawContext dc) {
		...
		// Draw the contents using the cached texture, if we've rendered to a
		// texture. Otherwise, just draw the
		// contents directly. Always draw the contents directly in picking mode
		// because unique pick colors can't be
		// cached in a texture.
	...
	protected void drawContentDirect(DrawContext dc) {
		...
		try {
			...
			gl.glScissor(this.contentBounds.x, this.contentBounds.y - 1,
					this.contentBounds.width + 1, this.contentBounds.height);

			this.contents.renderScrollable(dc,
					this.scrollContentBounds.getLocation(),
	...
	protected void drawContentTiles(DrawContext dc) {
		...
		try {
			...
			for (ContentTile tile : tiles) {
				...
				Rectangle tileScreenBounds = this.getContentTileBounds(
						tile.row, tile.column);
				Rectangle clippedTileBounds = tileScreenBounds
						.intersection(this.contentBounds);

				// If the tile is not visible in the content area, don't bother
				// drawing it.
				if (clippedTileBounds.isEmpty()) {
					continue;
				}

				Rectangle subTileBounds = new Rectangle(tileX
						+ clippedTileBounds.x - tileScreenBounds.x, tileY
						+ clippedTileBounds.y - tileScreenBounds.y,
						clippedTileBounds.width, clippedTileBounds.height);

				gl.glPushMatrix();
				try {
					gl.glTranslated(clippedTileBounds.x, clippedTileBounds.y,
							0.0f);

					gl.glColor4f(1, 1, 1, (float) this.getActiveAttributes()
							.getForegroundOpacity());

					TextureCoords texCoords = texture.getSubImageTexCoords(
							(int) subTileBounds.getMinX(),
							(int) subTileBounds.getMinY(),
							(int) subTileBounds.getMaxX(),
							(int) subTileBounds.getMaxY());
		...
		try {
			...
			if (!dc.isPickingMode()) {
				...
				try {
					...
					FrameFactory.drawBuffer(dc, GL.GL_LINE_STRIP,
							this.vertexBuffer);
				} finally {
				...
				gl.glLoadIdentity();
				gl.glTranslated(this.innerBounds.x, this.innerBounds.y, 0.0); // Translate
																				// back
																				// inner
																				// frame

				TreeUtil.drawRectWithGradient(gl, new Rectangle(0, 0,
						this.innerBounds.width, this.innerBounds.height),
						color[0], color[1], attributes.getBackgroundOpacity(),
						AVKey.VERTICAL);
			} else {
				...
				TreeUtil.drawPickableRect(dc, this.pickSupport, this,
				...
				if (this.isEnableResizeControl() && !this.isDrawMinimized()) {
					...
					this.pickSupport.addPickableObject(colorCode,
							this.frameResizeControl);
					gl.glColor3ub((byte) color.getRed(),
							(byte) color.getGreen(), (byte) color.getBlue());

					// Draw the resize control as a pickable line on the frame
					// border
				...
				// If both scroll bars are visible, draw the empty square in the
				// lower right hand corner as a part of
				// the resize control, pickable area.
				if (drawVerticalScrollbar && drawHorizontalScrollbar
						&& !this.isDrawMinimized()) {
			...
			if (this.isDrawTitleBar()) {
				gl.glTranslated(0, this.innerBounds.height
						- this.titleBarHeight, 0);
				this.drawTitleBar(dc);
			}

			// Draw a thin border outlining the filled rectangle that is the
			// frame background.
			if (!dc.isPickingMode()) {
				gl.glLoadIdentity();
				...
				// Do not draw the outline on the edges with scroll bars because
				// the scrollbar draws its own border. On
				// some devices the scroll bar border draws next to the frame
				// border, resulting in a double width border.
	...
	protected void drawScrollBars(DrawContext dc) {
		// Draw a vertical scroll bar if the tree extends beyond the visible
		// bounds
		if (this.showVerticalScrollbar) {
			...
			Rectangle scrollBarBounds = new Rectangle(x1, y1,
					this.scrollBarSize, this.contentBounds.height + 1);

			this.verticalScrollBar.setBounds(scrollBarBounds);
			this.verticalScrollBar.render(dc);
		}

		// Draw a horizontal scroll bar if the tree extends beyond the visible
		// bounds
		if (this.showHorizontalScrollbar) {
			int x1 = 1;
			int y1 = 1;
			...
			Rectangle scrollBarBounds = new Rectangle(x1, y1, width,
					this.scrollBarSize);
	...
	protected void drawTitleBar(DrawContext dc) {
		...
		if (!dc.isPickingMode()) {
			...
			TreeUtil.drawRectWithGradient(gl, new Rectangle(0, 0,
					this.innerBounds.width, this.getTitleBarHeight()),
					color[0], color[1], attributes.getBackgroundOpacity(),
					AVKey.VERTICAL);
	 ...
	 *            Draw context
	 * @param drawPoint
	 *            Point at which to draw the icon. This point is relative to the
	 *            lower left corner of the title bar. This point will be
	 *            modified to indicate how much horizontal space was consumed by
	 *            drawing the icon. After drawing the icon, the x value with
	 *            point to the first available space to the right of the icon.
	 */
	protected void drawIcon(DrawContext dc, Point drawPoint) {
		// This method is never called during picked, so picking mode is not
		// handled here
		...
		try {
			if (texture.bind(dc)) {
				...
				gl.glTranslated(drawPoint.x + iconSpace, drawPoint.y
						+ vertAdjust + 1, 1.0);
				gl.glScaled((double) iconSize.width, (double) iconSize.height,
						1d);
				dc.drawUnitQuad(texCoords);
	 ...
	 *            Draw context
	 * @param drawPoint
	 *            Point at which to draw text. This point is relative to the
	 *            lower left corner of the title bar.
	 */
	protected void drawTitleText(DrawContext dc, Point drawPoint) {
		...
		TextRenderer textRenderer = OGLTextRenderer.getOrCreateTextRenderer(
				dc.getTextRendererCache(), attributes.getFont());

		// Determine if the shortened frame title needs to be regenerated. If
		// so, generate it now.
		int titleAreaWidth = this.innerBounds.width - this.buttonSize
				- drawPoint.x - attributes.getIconSpace();
		...
		try {
			...
			double vertAdjust = (this.titleBarHeight - Math
					.abs(this.shortTitleBounds.getY())) / 2;
			textRenderer.draw(this.shortTitle, drawPoint.x,
					(int) (drawPoint.y + vertAdjust) + 1);
		} finally {
	 ...
	 * @param titleFont
	 *            Title font.
	 * @param titleAreaWidth
	 *            Width in pixels of the frame title area.
	 * 
	 ...
	 * method sets the fields {@link #shortTitle}, {@link #shortFrameTitleFont},
	 * {@link #frameTitleWidth}.
	 * 
	 * @param dc
	 *            Current draw context.
	 * @param frameTitle
	 ...
	 *            Width in pixels of the frame title area.
	 * @param cutOff
	 *            String to append to title to indicate that text has been cut
	 *            off due to a small frame. For example, if the cut off string
	 *            is "...", the string "Frame Title" might be shortened to
	 *            "Frame T...".
	 */
	protected void generateShortTitle(DrawContext dc, String frameTitle,
			int width, String cutOff) {
		Font font = this.getActiveAttributes().getFont();

		// Keep track of the font and width used to generate the title so that
		// we can invalidate the generated
		...
		// Check to see if the frame is too small to display even the
		// continuation
		...
		// Starting at the end of the string, remove characters until the string
		// fits
		...
		// Make sure that the computed string contains at least one character of
		// the original title. If not, don't
		// show any text.
	...
	protected void drawMinimizeButton(DrawContext dc) {
		...
		try {
			...
			int x = (int) this.innerBounds.getMaxX()
					- this.getActiveAttributes().getIconSpace()
					- this.buttonSize;
			int y = (int) this.innerBounds.getMaxY()
					- (this.titleBarHeight - this.buttonSize) / 2
					- this.buttonSize;
			gl.glTranslated(x, y, 0.0);

			if (!dc.isPickingMode()) {
				Color color = this.getActiveAttributes()
						.getMinimizeButtonColor();

				FrameAttributes attributes = this.getActiveAttributes();
				OGLUtil.applyColor(gl, color,
						attributes.getForegroundOpacity(), false);
				...
				gl.glBegin(GL2.GL_LINES);
				try {
					// Draw a horizontal line. If the frame is maximized, this
					// will be a minus sign. If the tree is
					...
					// Draw a vertical line to complete the plus sign if the
					// frame is minimized.
					if (this.isMinimized()) {
						gl.glVertex2f(buttonSize / 2f, buttonSize / 4f);
						gl.glVertex2f(buttonSize / 2f, buttonSize - buttonSize
								/ 4f);
					}
				} finally {
					gl.glEnd();
				...
				this.pickSupport.addPickableObject(colorCode,
						this.minimizeButton, null, false);
	...
	protected void beginDrawing(DrawContext dc) {
		...
		this.BEogsh.pushAttrib(gl, GL2.GL_DEPTH_BUFFER_BIT
				| GL2.GL_COLOR_BUFFER_BIT | GL2.GL_ENABLE_BIT
				| GL2.GL_CURRENT_BIT | GL2.GL_POLYGON_BIT // For polygon mode
				| GL2.GL_LINE_BIT // For line width
				| GL2.GL_TRANSFORM_BIT | GL2.GL_SCISSOR_BIT);
	...
	protected void determineScrollbarAttributes() {
		this.verticalScrollBar.setLineColor(this.activeAttributes
				.getForegroundColor());
		this.verticalScrollBar.setOpacity(this.activeAttributes
				.getBackgroundOpacity());
		this.horizontalScrollBar.setLineColor(this.activeAttributes
				.getForegroundColor());
		this.horizontalScrollBar.setOpacity(this.activeAttributes
				.getBackgroundOpacity());

		Color[] scrollBarColor = this.activeAttributes.getScrollBarColor();
		this.horizontalScrollBar.setKnobColor(scrollBarColor[0],
				scrollBarColor[1]);
		this.verticalScrollBar.setKnobColor(scrollBarColor[0],
				scrollBarColor[1]);
	}
	...
	protected void determineSize() {
		// Use the minimized size if the frame is minimized or animating to or
		// from the minimized state.
		if ((this.isMinimized() || this.isAnimating())
				&& this.minimizedSize != null) {
	...
	protected BasicWWTexture initializeTexture() {
		...
		if (imageSource instanceof String || imageSource instanceof URL) {
			URL imageURL = WorldWind.getDataFileStore().requestFile(
	 ...
	 * 
	 * @param direction
	 *            Determines which scroll bar to get. Either
	...
	protected void beginDrag(Point point) {
		if (this.isEnableMove()) {
			...
			this.dragRefPoint = new Point((int) location.getX() - point.x,
					(int) location.getY() - point.y);
		}
	}
	...
	public void drag(Point point) {
		if (this.isEnableMove()) {
			...
			this.setScreenLocation(new Offset(x, y, AVKey.PIXELS,
					AVKey.INSET_PIXELS));
		}
	}
	...
	public void selected(SelectEvent event) {
		...
		Rectangle titleBarBounds = new Rectangle(
				(int) this.awtScreenPoint.getX() + this.frameBorder,
				(int) this.awtScreenPoint.getY() + this.frameBorder * 2,
				this.innerBounds.width, this.titleBarHeight);

		if (event.isLeftDoubleClick()) {
			Point pickPoint = event.getPickPoint();
			if (pickPoint != null
					&& titleBarBounds.contains(event.getPickPoint())) {
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
		...
		// Java on Mac OS X implements support for horizontal scrolling by
		// sending a Shift+ScrollWheel event. This is
		// not the case for Java for other platforms, so we handle the scrolling
		// logic for Mac OS X
	 ...
	 * 
	 * @param e
	 *            Mouse event that triggered the scroll.
	 */
	protected void doScroll(MouseWheelEvent e) {
		// Determine whether to scroll horizontally or vertically by giving
		// priority to the vertical scroll bar. Scroll
		// vertically if only both scroll bars are active or only the vertical
		// scroll bar is active. Scroll horizontally
		// if only the horizontal scroll bar is active.
		if (this.showVerticalScrollbar) {
			this.verticalScrollBar.scroll(e.getUnitsToScroll()
					* this.getMouseWheelScrollUnit(AVKey.VERTICAL));
		} else if (this.showHorizontalScrollbar) {
			this.horizontalScrollBar.scroll(e.getUnitsToScroll()
	...
	protected void doScrollMacOS(MouseWheelEvent e) {
		// On Mac OS X, Java always scrolls horizontally when the Shift key is
		// down. This policy is used to support the
		// Magic Mouse and Magic Trackpad devices. When the user scroll
		// horizontally on either of these devices, Java
		// automatically sends a Shift+ScrollWheel event, regardless of whether
		// the Shift key is actually down. See
		// Radar #4631846 in
		...
		// If the Shift key is not down, Java Mac OS X implements the standard
		// scrolling policy used by Java on other
		// operating systems.
		else {
			this.doScroll(e);
		}
	...
	class ContentTile {
		 ...
		 * 
		 * @param row
		 *            Row of this tile in the frame content.
		 * @param column
		 *            Column of this tile in the frame content.
		 */
	...
	class TextureTile implements Comparable<TextureTile> {
		 ...
		 * 
		 * @param row
		 *            Row of the tile in the frame's backing texture.
		 * @param column
		 *            Column of the tile in the frame's backing texture.
		 */
		 ...
		 * 
		 * @param that
		 *            Tile to compare with.
		 * 
		 * @return -1 if this tile was accessed less recently than that tile, 0
		 *         if the access times are the same, or 1 if this tile was
		...
		public int compareTo(TextureTile that) {
			...
			return this.lastUsed < that.lastUsed ? -1
					: this.lastUsed == that.lastUsed ? 0 : 1;
		}
	}
}
